...
*22 error* cases listed by paratext, are being *accepted* by our system. 
...
1. because we dont ensure that _\\nb_ comes at the right place in a file. 
2. because, Same character style is closed and reopened
3. because, footnote comes nested within a character marker
4. character beg/end markers without space around them
5. a verseInwrongPlace error, where we can't see whats wrong
6. because we dont check if the column numbers are in order within the tr and tcr markers in a table
7. because we dont cross-check the number of values in gloss attribute of _\\rb_ marker with the number of words it encloses
8. we dont check for USFM 2.0 compliance
9. because, we treat the start and end markers of milestones as separate entities and do not ensure they occur in pair
10. because, we treat the start and end markers of milestones as separate entities and do not ensure they occur in pair with same ids
11. because, we treat the start and end markers of milestones as separate entities and do not ensure they occur in pair
12. (12-14)Glossary related constraints that looks like additionally added rules. Also it looks like they are cross checking the keyterm(_\\k_) with a Bible book's glossary outside of the file            - GlossaryCitationFormEndsInSpace error
...
* _\\id_, _\\c_, and _\\v_ markers are mandatory
...
* we strictly checks for a BCV structure in the document. So a _\\p_ or another indentation marker with text content, which is not under another marker to which the text belongs to(like _\\v_ or _\\ip_ ), is not accepted
