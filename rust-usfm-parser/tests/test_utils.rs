...
use std::path::Path;
use regex::Regex;
use rust_usfm::parser::{USFMParser,Filter};  // Adjust the import based on your actual module structure
...
impl USFMMarker {
    fn get_markers(&self) -> Vec<&'static str> {
        match self {
            ...
            USFMMarker::Titles => vec!["mt", "mte", "cl", "cd", "ms", "mr", "s", "sr", "r", "d", "sp", "sd"],
            USFMMarker::Comments => vec!["sts", "rem", "lit", "restore"],
            USFMMarker::Paragraphs => vec!["p", "m", "po", "pr", "cls", "pmo", "pm", "pmc"],
            USFMMarker::Characters => vec!["add", "bk", "dc", "ior", "iqt", "k", "litl", "nd", "ord", "pn"],
...
pub fn initialise_parser<P: AsRef<Path>>(input_usfm_path: P) -> Result<USFMParser, Box<dyn std::error::Error>> {
...
pub fn find_all_markers<P: AsRef<Path>>(
    ...
    if markers.contains("esbe") {
        assert!(markers.contains("esb"), "esb marker not found when esbe is present");
        markers.remove("esbe");
    }
    markers.remove("usfm");
...
pub fn is_valid_usfm<P: AsRef<Path>>(input_usfm_path: P) -> Result<bool, Box<dyn std::error::Error>> {
    // Check override list first
    let path_str = input_usfm_path.as_ref()
        .to_str()
...
fn test_usj_all_markers_are_in_output() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for file_path in test_files {
        ...
        assert!(usfm_parser.errors.is_empty(), "Parser errors: {:?}", usfm_parser.errors);
...
fn test_usj_output_is_valid() -> Result<(), Box<dyn std::error::Error>> {
   ...
   let compiled_schema = jsonschema::JSONSchema::compile(&schema)
       .expect("Failed to compile USJ schema");
...
fn test_compare_usj_with_testsuite_samples() -> Result<(), Box<dyn std::error::Error>> {
   ...
   for file_path in test_files.iter() {
       ...
       let usx_file_path = file_path.to_str().unwrap()
       ...
       let usj_file_path = file_path.to_str().unwrap()
