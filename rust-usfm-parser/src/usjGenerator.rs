...
"#;
    ...
    while cursor.goto_next_sibling() {
        let child = cursor.node();
            if child.kind() == "v" {
                println!("Chapter number before passing to verse: {:?}", global_chapter_number);
...
"#;
    ...
    let mut alt_number = None;
    let mut verse_text = String::new(); // To hold the verse text
    ...
    while let Some(capture) = captures.next() {
        ...
        if let Some(vp_capture) = capture.captures.get(2) {
            if let Ok(vp) = vp_capture.node.utf8_text(usfm.as_bytes()) {
                verse_text = vp.to_string();
            }
        }
    }
    ...
    let mut verse_json_obj = json!({
        ...
        "sid": format!(
            "GEN {}:{}",
            chapter_number.as_ref().map_or("Unknown", |c| c.as_str()),
