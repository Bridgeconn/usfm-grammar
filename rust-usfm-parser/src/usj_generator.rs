...
fn traverse_node(
    ...
    match node_type {
        "File" => {node_2_usj_id(&node, content, usfm, parser);}
        ...
        */
        "chapter" => {node_2_usj_chapter(&node, content, usfm, parser);}

        "cl" | "cp" | "cd" | "vp" | "\\" | "" => {node_2_usj_generic(&node, content, usfm, parser);}

        "ca" | "va" => {node_2_usj_ca_va(&node, content, usfm, parser);}

        "v" => {node_2_usj_verse(&node, content, usfm, parser);}

        "verseText" => {//node_2_usj_id(&node, content, usfm, parser);
        ...
        "paragraph" | "pi" | "ph" => {node_2_usj_para(&node, content, usfm, parser);}

        _ if NOTE_MARKERS.contains(&node_type) => {node_2_usj_notes(&node, content, usfm, parser);}

       /*  _ if CHAR_STYLE_MARKERS+NESTED_CHAR_STYLE_MARKERS+["xt_standalone", "ref"]=>{
            println!("CHAR:{}",node_text);

        }*/
        "Attribute" => {node_2_usj_attrib(&node, content, usfm, parser);}

        "text" => {//node_2_usj_text(&node, content, usfm, parser);
            let node_text = node
                .utf8_text(usfm.as_bytes())
                .expect("Failed to get node text")
                .to_string();
        ...
        "table" | "tr" => {node_2_usj_table(&node, content, usfm, parser);}
      
        "zNameSpace" | "milestone" => {node_2_usj_milestone(&node, content, usfm, parser);}

        "esb" | "cat" | "fig" => {node_2_usj_special(&node, content, usfm, parser);}
...
fn node_2_usj_special(
    ...
    let node_type=node.kind();
    match node_type {
        "esb" => {node_2_usj_id(&node, content, usfm, parser);
    ...
    "#;
        ...
        "fig" => {node_2_usj_id(&node, content, usfm, parser);
