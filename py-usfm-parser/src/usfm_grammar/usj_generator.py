...
class USJGenerator:
    ...
    def node_2_usj_id(self, node, parent_json_obj):
        ...
        for capture in id_captures:
            ...
            if capture_name == 'book-code':
                code = self.usfm[node_obj.start_byte:node_obj.end_byte].decode('utf-8')
            elif capture_name == 'desc':
                desc = self.usfm[node_obj.start_byte:node_obj.end_byte].decode('utf-8')
    ...
    def node_2_usj_c(self, node, parent_json_obj):
        ...
        for cap in chap_cap:
            ...
            if capture_name == 'alt-num':
                chap_json_obj['altnumber'] = self.usfm[node_obj.start_byte:node_obj.end_byte].decode('utf-8').strip()
            if capture_name == 'pub-num':
                chap_json_obj['pubnumber'] = self.usfm[node_obj.start_byte:node_obj.end_byte].decode('utf-8').strip()
    ...
    def node_2_usj_verse(self, node, parent_json_obj):
        ...
        verse_num = self.usfm[verse_node.start_byte:verse_node.end_byte].decode('utf-8').strip()
        ...
        for cap in verse_num_cap:
            node_obj, capture_name = cap
            text = self.usfm[node_obj.start_byte:node_obj.end_byte].decode('utf-8').strip()
            if capture_name == 'alt':
        ...
        if self.parse_state.get('book_slug') and self.parse_state.get('current_chapter'):
            v_json_obj['sid'] = f"{self.parse_state['book_slug']} {self.parse_state['current_chapter']}:{verse_num}"
    ...
    def node_2_usj_ca_va(self, node, parent_json_obj):
        ...
        alt_num = self.usfm[alt_num_match[0]['node'].start_byte:alt_num_match[0]['node'].end_byte].strip()
    ...
    def node_2_usj_para(self, node, parent_json_obj):
        ...
        if node.children and node.children[0].type.endswith('Block'):
            ...
            if para_marker == 'b':
                parent_json_obj['content'].append({'type': 'para', 'marker': para_marker})
            ...
            para_marker = self.usfm[node.children[0].start_byte:node.children[0].end_byte].decode('utf-8').replace('\\', '').strip()
            para_json_obj = {'type': 'para', 'marker': para_marker, 'content': []}
    ...
    def node_2_usj_notes(self, node, parent_json_obj):
        ...
        style = self.usfm[tag_node.start_byte:tag_node.end_byte].decode('utf-8').replace('\\', '').strip()
        ...
        note_json_obj['caller'] = self.usfm[caller_node.start_byte:caller_node.end_byte].decode('utf-8').strip()
    ...
    def node_2_usj_char(self, node, parent_json_obj):
        ...
        style = self.usfm[tag_node.start_byte:tag_node.end_byte].decode('utf-8').replace('\\', '').replace('+', '').strip()
        char_json_obj = {
            'type': 'char',
            'marker': style.strip(),
            'content': []
    ...
    def node_2_usj_attrib(self, node, parent_json_obj):
        ...
        attrib_name = self.usfm[attrib_name_node.start_byte:attrib_name_node.end_byte].decode('utf-8').strip()
        ...
        if attrib_val_cap:
            ...
            attrib_value = self.usfm[node_obj.start_byte:node_obj.end_byte].decode('utf-8').strip()
    ...
    def node_2_usj_milestone(self, node, parent_json_obj):
        ...
        style = self.usfm[ms_node.start_byte:ms_node.end_byte].decode('utf-8').replace('\\', '').strip()
    ...
    def node_2_usj_special(self, node, parent_json_obj):
        ...
        if node.type == 'esb':
            ...
            category = self.usfm[cat_node.start_byte:cat_node.end_byte].decode('utf-8').strip()
    ...
    def node_2_usj_generic(self, node, parent_json_obj):
        ...
        for i in range(children_range_start, len(node.children)):
            ...
            if any(child.type in marker_set for marker_set in [
    ...
    def push_text_node(self, node, parent_json_obj):
        ...
        text_val = self.usfm[node.start_byte:node.end_byte].decode('utf-8').replace('~', ' ')
        if text_val:
    ...
    def populate_dispatch_map(self):
        ...
        add_handlers(self.marker_lists['char_style'] + self.marker_lists['nested_char_style'] + ['xt_standalone'], self.node_2_usj_char)
