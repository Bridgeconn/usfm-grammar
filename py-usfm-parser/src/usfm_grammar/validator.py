...
class Validator:
            ...
            if err.is_missing:
                ...
                err_lines.append(
                    (
                        ...
                        self.usfm_bytes[err.start_byte : err.end_byte].decode("utf-8"),
                    )
                )
    ...
    def auto_fix_usfm(self, usfm, fixed=False):  # pylint: disable=R0912,R0915
        ...
        for error in self.usfm_errors:
            ...
            if (
                error.is_error
                and error_text.startswith("\\s5")
                and "paragraph" not in [ch.type for ch in error.children]
            ):
                # print("match 1")
                self.modified_usfm = re.sub(
                    r"\\s5[\s\n\r]*", r"\\s5 \n\\p\n", self.modified_usfm
                )
                changed = True
            ...
            # \p not given after section heading
                ...
                error.is_error
                and error_text.startswith("\\v")
                and error.parent.type == "s"
                and "paragraph" not in [ch.type for ch in error.children]
            ):
                # print("match 4")
            ...
            # space missing between \v and number
                ...
                # print("match 5")
                self.modified_usfm = re.sub(
                    v_without_space_pattern, r"\1 \2", self.modified_usfm
                )
                changed = True
            # space missing between \c and number
                ...
                # print("match 6")
                self.modified_usfm = re.sub(
                    c_without_space_pattern, r"\1 \2", self.modified_usfm
                )
                changed = True
            # \p not given at chapter start
                ...
                error.is_error
                and error_text.startswith("\\v")
                and error.prev_sibling.type == "chapter"
                and "paragraph" not in [ch.type for ch in error.children]
            ):
                # print("match 7")
            ...
            # Stray slash not with a valid marker
                ...
                self.modified_usfm = self.modified_usfm.replace(
                    to_replace, to_replace[1:]
                )
                changed = True
            # Just a single problematic marker (could be w/o text)
            elif error_text.startswith("\\") and re.match(
                valid_markers_pattern, error_text
            ):
                # print("Match 9")
            ...
            # Stray content in the chapter line
                ...
                error.parent.type == "chapter"
                and error.prev_sibling.type == "c"
                and "\\" not in error_text
            ):
                # print("Match 10")
...
valid_markers_pattern = re.compile(
    r"(\\id|\\usfm|\\ide|\\ref|\\h|\\toc|\\toca|\\sts|\\rem|"
