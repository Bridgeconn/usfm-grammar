...
@pytest.mark.parametrize(
    "exclude_markers",
    [["v", "c"], Filter.PARAGRAPHS, Filter.TITLES + Filter.BOOK_HEADERS],
)
@pytest.mark.timeout(30)
...
def test_usj_all_markers_are_in_output(file_path):
    ...
    for marker in all_markers_in_input:
        ...
        #     marker = "milestone"
        assert marker in all_json_types, print(
            f"{marker} not in {all_json_types} with all input of {all_markers_in_input}"
        )
...
def test_compare_usj_with_testsuite_samples(file_path):
    ...
    if usx_file_path not in exclude_USX_files:
        usj_dict = test_parser.to_usj()
        remove_newlines_in_text(
            usj_dict
        )  # need this if using USJ generated from tcdocs
        try:
            usj_file_path = file_path.replace("origin.usfm", "origin.json")
