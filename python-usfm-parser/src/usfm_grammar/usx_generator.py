...
class USXGenerator:
    ...
    def node_2_usx_chapter(self, node, usfm_bytes, parent_xml_node):
        ...
        for child in node.children:
            if child.type == "c":
                chap_ref = self.node_2_usx_c(child, usfm_bytes,parent_xml_node)
            else:
    ...
    def node_2_usx_verse(self, node, usfm_bytes, parent_xml_node):
    ...
    def node_2_usx_ca_va(self, node, usfm_bytes, parent_xml_node):
    ...
    def node_2_usx_para(self, node, usfm_bytes, parent_xml_node):
        ...
        if node.children[0].type.endswith('Block'):
            ...
            if not para_marker.endswith("Block"):
                ...
                for child in para_tag_cap[0].children[1:]:
                    self.node_2_usx(child, usfm_bytes, para_xml_node)
    ...
    def node_2_usx_notes(self, node, usfm_bytes, parent_xml_node):
        ...
        for child in node.children[2:-1]:
            self.node_2_usx(child, usfm_bytes, note_xml_node)

    def node_2_usx_char(self, node, usfm_bytes, parent_xml_node):
        ...
        for child in node.children[1:children_range]:
            self.node_2_usx(child, usfm_bytes, char_xml_node)

    def node_2_usx_attrib(self, node, usfm_bytes, parent_xml_node):
    ...
    def node_2_usx_table(self, node, usfm_bytes, parent_xml_node):
        ...
        if node.type == "table":
            ...
            for child in node.children[1:]:
                self.node_2_usx(child, usfm_bytes, cell_xml_node)
    ...
    def node_2_usx_milestone(self, node, usfm_bytes, parent_xml_node):
        ...
        for child in node.children:
            if child.type.endswith("Attribute"):
                self.node_2_usx(child, usfm_bytes, ms_xml_node)

    def node_2_usx_special(self, node, usfm_bytes, parent_xml_node):
        ...
        if node.type == "esb":
            ...
            for child in node.children[1:-1]:
                self.node_2_usx(child, usfm_bytes, fig_xml_node)
        elif node.type == 'b':
            etree.SubElement(parent_xml_node, "optbreak")

    def node_2_usx_generic(self, node, usfm_bytes, parent_xml_node):
    ...
    def node_2_usx(self, node, usfm_bytes, parent_xml_node): # pylint: disable= too-many-branches
        '''check each node and based on the type convert to corresponding xml element'''
        # print("working with node: ", node, "\n")
        if node.type == "id":
            self.node_2_usx_id(node, usfm_bytes, parent_xml_node)
        elif node.type == "chapter":
            self.node_2_usx_chapter(node, usfm_bytes,parent_xml_node)
        elif node.type in ["cl", "cp", "cd", "vp"]:
            self.node_2_usx_generic(node, usfm_bytes, parent_xml_node)
        elif node.type in ["ca", "va"]:
            self.node_2_usx_ca_va(node, usfm_bytes, parent_xml_node)
        elif node.type == "v":
            self.node_2_usx_verse(node, usfm_bytes, parent_xml_node)
            ...
            self.node_2_usx_table(node, usfm_bytes, parent_xml_node)
        elif  node.type == "milestone":
            self.node_2_usx_milestone(node, usfm_bytes, parent_xml_node)
        elif node.type == "zNameSpace":
            self.node_2_usx_milestone(node, usfm_bytes, parent_xml_node)
        elif node.type in ["esb", "cat", "fig", "b"]:
            self.node_2_usx_special(node, usfm_bytes, parent_xml_node)
